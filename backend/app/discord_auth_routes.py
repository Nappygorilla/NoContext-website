...
def register_discord_auth_routes(app, engine, set_session, User):
    ...
    def redirect_uri() -> str:
        # Support both names so older Render deployments keep working.
        return (
    ...
    def discord_request(
        ...
        try:
            ...
            # Keep the user-facing message generic, but log Discord's status/body
            # so the backend can be diagnosed without exposing client secrets.
            try:
            ...
            logger.warning("Discord OAuth request failed: method=%s url=%s error=%s", method, url, exc)
            raise HTTPException(
                status_code=502,
                detail="Discord authentication is temporarily unavailable.",
            ) from exc
    ...
    def discord_login_callback(code: str, state: str):
        ...
        with Session(engine) as db:
            saved = db.scalar(
                select(DiscordAuthState).where(DiscordAuthState.state_hash == sha(state.strip()))
            )
            if not saved:
        ...
        discord_username = str(
            identity.get("global_name") or identity.get("username") or "Discord User"
        ).strip()[:128]
        ...
        response = RedirectResponse(
            frontend() + "/NoContext-website/account-dashboard.html",
            status_code=302,
        )
