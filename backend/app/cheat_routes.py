...
class CheatBase(DeclarativeBase): pass
...
def register_cheat_routes(app,engine,session_from_request,require_csrf):
    ...
    with Session(engine) as db:
        for slug,name,status in [("external","NoContext External","online"),("executor","NoContext Executor","coming_soon")]:
            if db.scalar(select(CheatStatus).where(CheatStatus.slug==slug)) is None: db.add(CheatStatus(slug=slug,name=name,status=status))
        db.commit()
    def owner(request:Request):
        auth=session_from_request(request)
        if not auth: raise HTTPException(status_code=401,detail="Not signed in.")
        _,user=auth
        if user.id!=OWNER_ID: raise HTTPException(status_code=403,detail="Owner access required.")
        return user
    ...
    def update_cheat_status(slug:str,body:CheatStatusUpdate,request:Request):
        ...
        if status not in ALLOWED_STATUSES: raise HTTPException(status_code=422,detail="Invalid cheat status.")
        with Session(engine) as db:
            item=db.scalar(select(CheatStatus).where(CheatStatus.slug==slug.lower().strip()))
            if not item: raise HTTPException(status_code=404,detail="Cheat not found.")
