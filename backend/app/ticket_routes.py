...
def _send_discord(ticket: Ticket, user_name: str, message: str, event: str) -> None:
    if not DISCORD_WEBHOOK_URL:
        return
    payload = {"username": "NoContext Tickets", "embeds": [{"title": f"Ticket #{ticket.id} · {event}", "description": message[:4000], "fields": [{"name": "Subject", "value": ticket.subject[:1024], "inline": False}, {"name": "User", "value": f"{user_name} · ID {ticket.user_id}", "inline": True}, {"name": "Status", "value": ticket.status, "inline": True}], "footer": {"text": "NoContext support"}}]}
    try:
        request = urllib.request.Request(DISCORD_WEBHOOK_URL, data=json.dumps(payload).encode("utf-8"), headers={"Content-Type": "application/json", "User-Agent": "NoContext-Tickets/1.0"}, method="POST")
        with urllib.request.urlopen(request, timeout=5):
            pass
    except Exception:
        pass

def register_ticket_routes(app, engine, require_csrf, session_from_request, enforce_origin, rate_limit, user_model=None):
    ...
    def ticket_json(ticket: Ticket):
        return {"id": ticket.id, "userId": ticket.user_id, "subject": ticket.subject, "status": ticket.status, "discordThreadId": ticket.discord_thread_id, "createdAt": _utc(ticket.created_at), "updatedAt": _utc(ticket.updated_at)}

    def message_json(message: TicketMessage):
        return {"id": message.id, "ticketId": message.ticket_id, "authorUserId": message.author_user_id, "authorName": message.author_name, "body": message.body, "source": message.source, "createdAt": _utc(message.created_at)}
    ...
    def admin_overview(request: Request):
        ...
        with Session(engine) as db:
            users = db.execute(select(user_model)).scalars().all()
            tickets = db.scalars(select(Ticket).order_by(Ticket.updated_at.desc())).all()
            return {"ownerId": OWNER_ID, "users": [{"id": u.id, "username": u.username, "email": u.email, "createdAt": _utc(u.created_at)} for u in users], "tickets": [ticket_json(ticket) for ticket in tickets], "discordConfigured": bool(DISCORD_WEBHOOK_URL)}
