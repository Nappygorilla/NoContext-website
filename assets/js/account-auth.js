(() => {
    ...
    const clearAuthState = () => {
        ...
        // Remove legacy client-side auth artifacts from older builds. The real
        // session is an HttpOnly cookie and is never readable by JavaScript.
    ...
    const renderTickets = async () => {
        ...
        const openTicket = async id => {
            ...
            list.querySelector('[data-send-ticket-reply]')?.addEventListener('click', async e => {
                ...
                try { await request(`/api/tickets/${id}/messages`, { method: 'POST', body: JSON.stringify({ body }) }); await openTicket(id); }
        ...
        try {
            ...
            if (Number(data.user.id) === 1) {
                ...
                if (sidebar && !sidebar.querySelector('[data-admin-link]')) {
                    ...
                    link.className = 'account-sidebar-link'; link.href = 'admin-dashboard.html'; link.dataset.adminLink = 'true';
