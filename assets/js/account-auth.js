  ...
  const addMediaPanel = async () => {
    ...
    try {
      ...
      if (!app) {
        ...
        content.innerHTML = `<div class="license-box"><div style="display:flex;justify-content:space-between;gap:14px;align-items:flex-start;flex-wrap:wrap"><div><h3>Application ${app.status}</h3><p>${escapeHtml(app.status === "rejected" ? (app.reviewNote || "Your application was not approved.") : "Your application is in review. We will update this panel when it is reviewed.")}</p></div><a class="btn btn-outline" href="media-login.html">Media Login</a></div>${app.status === "rejected" ? `<p class="account-note">You can submit a new application after this review has been closed.</p>` : ""}</div>`;
      }
    } catch (error) {
