(() => {
  ...
  const request = async (path, options = {}) => {
    const res = await fetch(`${api}${path}`, { ...options, credentials:'include', cache:'no-store', headers:{'Content-Type':'application/json', Authorization:`Bearer ${token}`, ...(csrf ? {'X-CSRF-Token':csrf} : {}), ...(options.headers || {})} });
  ...
  const load = async () => {
    try {
      const data = await request('/api/admin/cheats');
      list.innerHTML = (data.statuses || []).map(item => `<div data-cheat="${esc(item.slug)}" style="padding:16px;border:1px solid rgba(255,255,255,.07);border-radius:14px;background:rgba(255,255,255,.02)"><div style="display:flex;justify-content:space-between;gap:12px;align-items:center;margin-bottom:12px"><div><strong>${esc(item.name)}</strong><div style="color:var(--text-muted);font-size:.75rem;margin-top:4px">/${esc(item.slug)}</div></div><span style="color:var(--accent);font-size:.72rem;text-transform:uppercase">${esc(item.status.replace('_',' '))}</span></div><div style="display:grid;grid-template-columns:1fr 1fr;gap:10px"><label style="font-size:.75rem;color:var(--text-muted)">Status<select data-status style="display:block;width:100%;margin-top:6px;background:#0b0d12;color:#fff;border:1px solid rgba(255,255,255,.1);border-radius:10px;padding:10px"><option value="online">Online</option><option value="updating">Updating</option><option value="offline">Offline</option><option value="maintenance">Maintenance</option><option value="coming_soon">Coming Soon</option></select></label><label style="font-size:.75rem;color:var(--text-muted)">Version<input data-version maxlength="64" value="${esc(item.version)}" placeholder="e.g. 1.0.0" style="display:block;width:100%;margin-top:6px;background:#0b0d12;color:#fff;border:1px solid rgba(255,255,255,.1);border-radius:10px;padding:10px;box-sizing:border-box"></label></div><label style="display:block;font-size:.75rem;color:var(--text-muted);margin-top:10px">Status note<textarea data-note maxlength="500" placeholder="Optional public status note…" style="display:block;width:100%;min-height:70px;margin-top:6px;background:#0b0d12;color:#fff;border:1px solid rgba(255,255,255,.1);border-radius:10px;padding:10px;box-sizing:border-box;font:inherit">${esc(item.note)}</textarea></label><div style="display:flex;justify-content:flex-end;align-items:center;gap:10px;margin-top:10px"><span data-message style="font-size:.75rem;color:var(--text-muted)"></span><button class="btn btn-primary" data-save type="button">Save Status</button></div></div>`).join('') || '<p>No products configured.</p>';
      (data.statuses || []).forEach(item => { const row=list.querySelector(`[data-cheat="${CSS.escape(item.slug)}"]`); if(!row)return; row.querySelector('[data-status]').value=item.status; row.querySelector('[data-save]').onclick=async()=>{const button=row.querySelector('[data-save]');const msg=row.querySelector('[data-message]');button.disabled=true;msg.textContent='Saving…';try{await request(`/api/admin/cheats/${encodeURIComponent(item.slug)}`,{method:'PATCH',body:JSON.stringify({status:row.querySelector('[data-status]').value,version:row.querySelector('[data-version]').value,note:row.querySelector('[data-note]').value})});msg.textContent='Saved';setTimeout(()=>{msg.textContent='';},1800);}catch(e){msg.textContent=e.message;button.disabled=false;}finally{button.disabled=false;}}; });
    } catch (e) { list.innerHTML=`<p class="admin-error">${esc(e.message)}</p>`; }
  };
  load();
})();
