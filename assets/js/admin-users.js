(() => {
  ...
  document.querySelector('[data-create-owner-key]')?.addEventListener('click', async () => {
    const button=document.querySelector('[data-create-owner-key]'); const duration=document.querySelector('[data-key-duration]')?.value; const product=document.querySelector('[data-key-product]')?.value || 'NoContext External'; button.disabled=true;
    try { const data=await request('/api/admin/keys',{method:'POST',body:JSON.stringify({duration,product})}); keyResult.hidden=false; keyResult.innerHTML=`<strong>Created: ${esc(data.durationLabel)}</strong><code>${esc(data.key)}</code><button type="button" class="btn btn-outline" data-copy-owner-key>Copy Key</button><small>Save this key now. The plaintext key is returned only at creation.</small>`; keyResult.querySelector('[data-copy-owner-key]').addEventListener('click',async e=>{await navigator.clipboard.writeText(data.key);e.currentTarget.textContent='Copied';}); } catch(error){keyResult.hidden=false;keyResult.innerHTML=`<span class="admin-error">${esc(error.message)}</span>`;} finally{button.disabled=false;}
  });
  if(root) load();
})();
