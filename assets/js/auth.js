(() => {
    ...
    const writeCsrf = value => { if (value) sessionStorage.setItem('nocontext_csrf', value); else sessionStorage.removeItem('nocontext_csrf'); };
    const clearSession = () => { sessionStorage.removeItem('nocontext_session'); sessionStorage.removeItem('nocontext_csrf'); localStorage.removeItem('nocontext_session_token'); localStorage.removeItem('nocontext_session'); localStorage.removeItem('nocontext_csrf'); };
    let csrfToken = readCsrf();
    let requestInFlight = false;
    const setStatus = (message, kind = 'error') => { let node = form.querySelector('.auth-status'); if (!node) { node = document.createElement('p'); node.className = 'auth-status'; node.setAttribute('role','status'); node.setAttribute('aria-live','polite'); form.appendChild(node); } node.textContent = message; node.dataset.kind = kind; };
    ...
    if (configured) {
        const wake = () => fetch(`${configured}/api/health`, { method:'GET', credentials:'omit', cache:'no-store' }).catch(() => {});
        if ('requestIdleCallback' in window) requestIdleCallback(wake, { timeout: 900 }); else setTimeout(wake, 150);
    }
    form.removeAttribute('novalidate');
    firstField?.focus({ preventScroll: true });
    const password = form.querySelector('#password');
    if (password && !password.parentElement?.querySelector('.password-toggle')) {
        const wrap = password.parentElement; wrap?.classList.add('password-field'); const toggle = document.createElement('button'); toggle.type='button'; toggle.className='password-toggle'; toggle.setAttribute('aria-label','Show password'); toggle.textContent='Show'; wrap?.appendChild(toggle);
    ...
    form.addEventListener('submit', async event => {
        ...
        if (isRegister) { const pass=form.querySelector('#password')?.value||''; const confirm=form.querySelector('#confirm')?.value||''; if(pass!==confirm){form.querySelector('#confirm')?.focus();setStatus('Passwords do not match.');return;} }
        ...
        } catch(error) { clearSession(); setStatus(error instanceof Error?error.message:'Unable to authenticate.'); requestInFlight=false; submit.disabled=false; submit.removeAttribute('aria-busy'); submit.textContent=originalText; }
    });
})();
