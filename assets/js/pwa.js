(() => {
    ...
    if ('serviceWorker' in navigator && location.protocol === 'https:') {
        navigator.serviceWorker.register('/NoContext-website/sw.js', { scope: '/NoContext-website/' }).catch(() => {});
    }
    ...
    document.querySelectorAll('a[href]').forEach(link => {
        ...
        link.addEventListener('pointerenter', () => { if (!reduced()) fetch(url.href, { cache: 'force-cache', credentials: 'same-origin' }).catch(() => {}); }, { passive: true, once: true });
    });

    const updateConnection = () => {
        document.documentElement.dataset.connection = navigator.onLine ? 'online' : 'offline';
        if (!navigator.onLine) toast('You are offline. Cached NoContext pages remain available.', 'offline');
    };
    addEventListener('online', () => toast('Connection restored.', 'success'));
