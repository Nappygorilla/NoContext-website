(() => {
    ...
    const toast = (message) => {
        ...
        if (!node) {
            node = document.createElement('div'); node.id = 'nc-toast'; node.setAttribute('role', 'status'); node.setAttribute('aria-live', 'polite');
    ...
    if (backend) {
        ...
        fetch(`${apiBase}/api/health`, { cache: 'no-store', credentials: 'omit' })
            ...
            .then(() => {
                ...
                const latencyLabel = document.getElementById('nc-latency'); if (latencyLabel) latencyLabel.textContent = `${latency}ms`;
                const uptime = document.getElementById('nc-uptime'); if (uptime) uptime.textContent = 'live';
            })
            .catch(() => {
                backend.textContent = 'Unavailable'; backend.classList.remove('online');
                const uptime = document.getElementById('nc-uptime'); if (uptime) uptime.textContent = 'offline';
            });
    }
    ...
    const syncNavigation = async () => {
        ...
        const api = String(window.NO_CONTEXT_API_URL || '').trim().replace(/\/$/, ''); if (!api) return;
        try {
            ...
            actions.innerHTML = signedIn ? '<a href="/NoContext-website/account-dashboard" class="btn btn-primary">Dashboard</a>' : '<a href="/NoContext-website/login" class="btn btn-outline">Sign In</a><a href="/NoContext-website/register" class="btn btn-primary">Register</a>';
        } catch (_) {}
    };
    syncNavigation();
    ...
    const updateConnection = () => { document.documentElement.dataset.connection = navigator.onLine ? 'online' : 'offline'; if (!navigator.onLine) toast('You are offline. Cached NoContext pages remain available.'); };
